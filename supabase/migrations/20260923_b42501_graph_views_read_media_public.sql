-- =============================================================================
-- B-42501 | knowledge_graph_nodes / knowledge_graph_edges → media_public
-- DRAFT ONLY — DO NOT APPLY without 极光 execution approval
-- Forbidden: GRANT SELECT ON public.media TO anon
-- Forbidden: altering media_public security_invoker (C3 separate)
-- Keep: security_invoker = true on both views
-- knowledge_recommendations: no definition change (depends on nodes/edges)
-- Source: production pg_get_viewdef 2026-09-23; minimal replace only
-- =============================================================================
...
COMMENT ON VIEW public.knowledge_graph_nodes IS
  'B-42501: reads media_public; security_invoker kept; no anon GRANT on media';
COMMENT ON VIEW public.knowledge_graph_edges IS
  'B-42501: reads media_public; security_invoker kept; no anon GRANT on media';

-- ROLLBACK: CREATE OR REPLACE VIEW ... WITH (security_invoker = true) AS
-- <body from pre-B-knowledge_graph_nodes.sql / pre-B-knowledge_graph_edges.sql>
