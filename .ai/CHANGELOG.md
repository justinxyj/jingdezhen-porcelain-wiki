...
## 2026-09-20 — 搜索发现控件与公开术语修复
- 统一搜索筛选控件增加原生链接兜底，并保留客户端筛选增强。
- 修复搜索示例按钮的点击路径。
- 将近期数字博物馆、关系探索、世界浏览中的公开“Entry”等产品术语替换为中文“知识条目”等。
- 未修改知识事实、关系数据、世界映射或推荐门槛。


## 2026-09-20 — 搜索控件第二轮修复
- 搜索示例与筛选项的原生链接统一指向 `#jdm-search-results`。
- 即使 JavaScript 尚未接管，导航后也会直接落到结果区。
- 保留现有 JavaScript 无刷新筛选逻辑。


## 2026-09-20 — Phase 4E-3 收口 / Phase 4E-4 启动
- Phase 4E-3 器物图谱 2.0 正式收口；器物作为统一知识发现入口稳定运行。
- Phase 4E-4 人物数据库 2.0 启动。
- 新增人物聚合层：知识世界、已有关系、工艺、时代、空间、代表作品、窑址、文献、关联人物与同时代人物。
- 人物数据库增加关键词 / 时代 / 身份 / 知识世界筛选。
- 人物卡片提供知识条目与全球网络继续探索。
- 本阶段仍不新增人物事实，不改变既有关系、世界映射与推荐门槛。


## 2026-09-20 — 时代分类统一修正
- 修正人物数据库时代下拉框：删除 20世纪、21世纪，新增近代，并统一为七级时代体系。
- 器物图谱、统一搜索、时间轴同步采用七级时代体系：唐五代、宋、元、明、清、近代、现代。
...
## 2026-09-20 — AI Long-Term Memory Daily Archive
- 归档今日完整项目工作快照至 PROJECT_MEMORY.md。
- 同步当前状态、任务冻结与后续接管边界。
- 记录用户确认的两个外部修复：技术树“历史”样式、窑址地图内容消失；后续不重复处理。
