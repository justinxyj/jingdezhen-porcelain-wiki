...
## Phase 3B-3 第四轮状态（2026-09-20）
- A+ 当前已出现 4 条：郎廷极→御窑厂遗址、年希尧→御窑厂遗址、臧应选→御窑厂遗址、黄云鹏→青花瓷。
- A 级保持上一轮及新增高价值关系；B 级仍用于“关系合理但证据不足”，C 级继续留在图谱层而不进入推荐。
...
## 当前修复状态
- 统一搜索筛选控件：已修复原生点击兜底 + JavaScript 事件委托。
- 搜索示例：已改为可直接导航的链接并由 JavaScript 接管。
- 用户界面不再以“Entry”作为公开产品术语。


## 当前搜索控件状态
- 原生链接增加结果区锚点，避免点击后视觉上“跳回顶部”。
- JavaScript 正常加载时仍使用无刷新动态筛选。


## Phase 4E-3 / 4E-4 当前状态
- Phase 4E-3 器物图谱 2.0 已正式收口。
- Phase 4E-4 人物数据库 2.0 第一轮已完成，等待用户线上浏览器 smoke。
- 人物公开数据仍为 51 个已发布人物条目。
- 人物聚合层仅读取既有生产数据，不创建第二事实源。
- 推荐门槛、Semantic Freeze、World mappings 均未改变。


## 时代分类当前状态
- 公开时代筛选标准：唐五代 / 宋 / 元 / 明 / 清 / 近代 / 现代。
- 人物数据库已移除 20世纪、21世纪选项。
- 器物图谱、统一搜索、时间轴与人物数据库使用同一时代分组逻辑。
