  ...
  function init(){
    ...
    boot.then(DATA=>{
      ...
      function select(id){
        ...
        const image=x.image_url?`<div class="tech-detail-media"><img src="${esc(x.image_url)}" alt="${esc(x.name_zh)}工序资料图" loading="lazy"><small>${esc(x.image_credit||'')}</small>${x.image_creator?`<small>作者：${esc(x.image_creator)}</small>`:''}${x.image_license?`<small>许可：${esc(x.image_license)}</small>`:''}${x.image_source_url?`<a href="${esc(x.image_source_url)}" target="_blank" rel="noopener">图片来源 ↗</a>`:''}</div>`:'<div class="tech-detail-media"><div class="tech-detail-no-image">暂无工序图片候选</div></div>';
        const badge=x.image_status==='verified'?'已核验图片':x.image_source_type==='official_stage_representative'?'官方阶段代表图':x.image_source_type==='commons_candidate'?'Wikimedia Commons 候选':x.image_source_type==='google_candidate'?'Google 图片候选':'待审核图片';\n        const candidateNote=x.image_search_query?`<div class="tech-detail-image-search"><b>补图检索：</b>${esc(x.image_search_query)}</div>`:'';
