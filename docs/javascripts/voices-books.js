...
(function(){
  ...
  function card(e,i,type){
    const m=meta(e),im=e.media?.[0],q=quoteOf(e),src=sourceOf(e,type),translation=m.quote_translation||m.translation||'';
    ...
    return `<article class="reception-card ${type==='person'?'reception-person-card':'reception-text-card'}" data-index="${i}"><div class="reception-card-main">${im?`<img class="reception-thumb" src="${esc(im.path)}" alt="${esc(im.title||title)}" loading="lazy">`:''}<div class="reception-context"><span class="reception-type">${relation}</span>${m.era||m.period?`<span class="reception-era">${esc(m.era||m.period)}</span>`:''}<blockquote>“${esc(plain(q))}”</blockquote>${translation?`<p class="reception-translation">${esc(plain(translation))}</p>`:''}<h3>${esc(title)}</h3><p class="reception-speaker">${esc(speaker)}</p>${m.quote_work?`<p class="reception-work">${esc(m.quote_work)}</p>`:''}<div class="reception-source"><span>来源</span>${src.url?`<a href="${esc(src.url)}" target="_blank" rel="noopener">${esc(src.label)} ↗</a>`:`<span>${esc(src.label)}</span>`}</div></div></div></article>`;
  }
  function init(){
    ...
    window.JDM_KNOWLEDGE.all().then(entries=>{
      ...
      root.innerHTML=`<div class="reception-intro"><span class="reception-kicker">VOICES · 评价史</span><h2>先读评价，再看出处。</h2><p>这里的主角是“评价”本身。书籍、人物与文献只是来源；页面首先呈现关于景德镇的原话。外文评价同时提供中文释义。</p></div><div class="reception-tabs"><button class="is-active" data-view="all">全部评价 · ${people.length+books.length}</button><button data-view="literature">书中评价 · ${books.length}</button><button data-view="person">名人评价 · ${people.length}</button></div><div class="reception-grid" id="reception-grid">${people.map((e,i)=>card(e,i,'person')).join('')}${books.map((e,i)=>card(e,people.length+i,'literature')).join('')}</div><div class="reception-empty" id="reception-empty" hidden>目前还没有可展示的评价。</div>`;
