/* Media policy: only verified, renderable media may reach public UI. */
(function(){
  window.JDM_MEDIA_POLICY={
    ...
    categoryRules:{
      ...
      '人物':'优先本人可靠肖像或官方机构人物资料图；无可靠肖像则不强行配图',
    ...
    isGenericPlaceholder(media){
      ...
      const p=String(media.path||'').trim();
      const t=String(media.title||'').trim();
      const s=String(media.source||'').trim();
      if(!p)return true;
      if(/视觉索引|关联图|占位|placeholder|待补|暂无|未核验|示意图|配图/i.test(t))return true;
      if(/placeholder|no[-_ ]image|noimage|blank|transparent/i.test(p))return true;
      if(/42490\/177595\/main-image/.test(p))return true;
      if(/关联图|视觉索引/i.test(s))return true;
      return false;
    },
    isUsable(media){return !this.isGenericPlaceholder(media);}
  };
})();
