...
(function(){
  ...
  function init(entries){
    ...
    function open(e){const m=e.zh?.meta?.map||{},wikiTitle=m.wikiTitle||e.zh?.title||e.slug,source=e.sources?.[0]?.url||'',modal=document.getElementById('global-kiln-modal');modal.innerHTML=`<div class="global-kiln-modal"><div class="global-kiln-backdrop" data-close></div><article class="global-kiln-dialog"><button class="global-kiln-close" data-close>×</button><div class="global-kiln-image">${mediaMarkup(e)}</div><div class="global-kiln-content"><div class="global-kiln-tags"><span>${esc(m.country||'')}</span><span>${esc(m.period||'')}</span><span>${esc(m.type||'窑业中心')}</span></div><h2>${esc(e.zh?.title||'窑址')}</h2><p class="global-kiln-intro">${esc(plain(e.zh?.content||e.zh?.summary||''))}</p><div class="global-kiln-ai"><b>摘要</b><p>${esc(plain(e.zh?.summary||''))}</p></div><div class="global-kiln-links"><a href="${entryUrl(e)}">查看详情 →</a><a href="${wiki(wikiTitle)}" target="_blank" rel="noopener">维基百科 ↗</a>${source?`<a href="${esc(source)}" target="_blank" rel="noopener">权威来源 ↗</a>`:''}</div></div></article></div>`;modal.querySelectorAll('[data-close]').forEach(x=>x.addEventListener('click',()=>{modal.innerHTML='';document.body.classList.remove('global-kiln-open')}));document.body.classList.add('global-kiln-open')}
    function plain(s){const d=document.createElement('div');d.innerHTML=String(s||'');return d.textContent||d.innerText||''}
    function render(){const q=(document.getElementById('global-kiln-search')?.value||'').trim().toLowerCase();const active=document.querySelector('.global-kiln-map-toolbar button.is-active')?.dataset.region||'all';markers.forEach((marker,e)=>{if(visible(e,q,active))marker.addTo(map);else map.removeLayer(marker)});const list=document.getElementById('global-kiln-list');list.innerHTML=rows.filter(e=>visible(e,q,active)).map(e=>{const m=e.zh.meta.map;return `<button class="global-kiln-list-item" data-slug="${esc(e.slug)}"><b>${esc(e.zh?.title||'窑址')}</b><span>${esc(m.country||'')} · ${esc(m.period||'')}</span></button>`}).join('');list.querySelectorAll('[data-slug]').forEach(btn=>btn.addEventListener('click',()=>{const e=rows.find(x=>x.slug===btn.dataset.slug);if(!e)return;const m=e.zh.meta.map;map.flyTo([m.lat,m.lng],Math.max(map.getZoom(),5),{duration:.7});open(e)}));resizeMap()}
