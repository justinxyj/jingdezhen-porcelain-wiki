...
(function(){
  ...
  function mediaMarkup(e){const im=(e.media||[]).find(m=>window.JDM_MEDIA_POLICY?.isUsable?window.JDM_MEDIA_POLICY.isUsable(m):Boolean(m?.path));return im?`<img src="${esc(im.path)}" alt="${esc(im.title||e.zh?.title||'窑址图片')}" loading="lazy"><small>${esc(im.title||'馆藏图片')}</small>`:'<span>暂无可用于地图展示的已核验遗址图</span>'}
