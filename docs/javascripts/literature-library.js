...
(function(){
  ...
  const sourceUrl=e=>(e.sources||[]).find(s=>s&&typeof s==='object'&&s.url)?.url||'';
  function init(){
    ...
    window.JDM_KNOWLEDGE.all().then(entries=>{
      ...
      root.innerHTML=docs.map(e=>{const m=e.zh.meta,source=sourceUrl(e);return `<article class="literature-item" data-category="${esc(m.libraryCategory||'')}" data-kind="${esc(m.kind||'reference')}"><b>${esc(m.ref)}</b><h3>${esc(e.zh?.title||e.slug)}</h3><p>${esc(plain(e.zh?.content||e.zh?.summary||''))}</p><div class="literature-actions"><a href="${window.JDM_KNOWLEDGE.url(e)}">进入统一知识条目 →</a>${source?`<a href="${esc(source)}" target="_blank" rel="noopener">在线资料 ↗</a>`:''}</div></article>`}).join('');
