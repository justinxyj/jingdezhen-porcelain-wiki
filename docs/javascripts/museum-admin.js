...
(function(){
  ...
  async function load(){
    ...
    const request=window.JDM_AUTH?.request||((factory)=>factory(db));
    const [entries,media,sources]=await Promise.all([
      request(d=>d.from('entries').select('id,slug,category,zh,status,confidence,editorial_status,reviewed_at').order('updated_at',{ascending:false})),
      request(d=>d.from('media').select('id,entry_id,path,title,source,license,status,verification_status,is_primary').order('created_at',{ascending:false})),
      request(d=>d.from('sources').select('id,title,url,tier,status,institution,last_checked_at').order('updated_at',{ascending:false}))
    ]);

    return {entries:entries||[],media:media||[],sources:sources||[]};
  }
  function checks({entries,media,sources}){
    ...
    const issues=[];
    entries.forEach(e=>{if(!e.zh?.title)issues.push(['entry','缺少标题',e.slug]);if(!String(e.zh?.content||'').trim())issues.push(['entry','缺少正文',e.slug]);if(!String(e.zh?.summary||'').trim())issues.push(['entry','缺少简介',e.slug]);if(!e.confidence||e.confidence==='unreviewed')issues.push(['review','尚未标注可信度',e.slug]);if(!mediaByEntry.has(e.id))issues.push(['media','没有审核通过的媒体',e.slug])});
    media.forEach(m=>{if(!String(m.path||'').trim())issues.push(['media','图片路径为空',m.id]);if(m.verification_status!=='verified')issues.push(['media',`图片状态：${mediaState(m.verification_status)}`,m.title||m.id])});
    sources.forEach(s=>{if(!String(s.title||'').trim())issues.push(['source','来源缺少标题',s.id]);if(!String(s.url||'').trim())issues.push(['source','来源缺少URL',s.title||s.id]);if(!s.last_checked_at)issues.push(['source','来源尚未检查',s.title||s.id])});
    return issues;
  }
