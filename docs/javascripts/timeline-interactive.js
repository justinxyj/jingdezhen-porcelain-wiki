...
(function(){
  ...
  function open(e){
    ...
    modal.innerHTML=`<div class="jdm-timeline-backdrop"></div><article class="jdm-timeline-dialog"><button class="jdm-timeline-close" aria-label="关闭">×</button><div class="jdm-timeline-detail">${im?`<div class="jdm-timeline-image"><img src="${esc(im.path)}" alt="${esc(im.title||e.zh?.title||e.slug)}"></div>`:''}<div class="jdm-timeline-copy"><div class="detail-overline">历史节点</div><h3>${esc(e.zh?.title||e.slug)}</h3><div class="detail-meta">${m.period?`<span>${esc(m.period)}</span>`:''}${m.map?.country?`<span>${esc(m.map.country)}</span>`:''}</div><p>${esc(text(e.zh?.content||''))}</p>${e.zh?.summary?`<div class="ai-summary"><b>简介</b><p>${esc(text(e.zh.summary))}</p></div>`:''}<div class="jdm-timeline-actions">${source?`<a class="secondary" href="${esc(source)}" target="_blank" rel="noopener">来源 ↗</a>`:''}<a class="secondary" href="https://zh.wikipedia.org/w/index.php?search=${encodeURIComponent(wikiTitle)}" target="_blank" rel="noopener">维基百科 ↗</a></div></div></div></article></div>`;
