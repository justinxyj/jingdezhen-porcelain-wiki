...
(function(){
  ...
  const text=e=>{const v=e?.zh?.summary||e?.zh?.content||'';const d=document.createElement('div');d.innerHTML=String(v);return d.textContent||d.innerText||''};
  const title=e=>e?.zh?.title||e?.slug||'';
  function node(e){const m=meta(e),map=m.map||{},im=e.media?.[0];return `<a class="compare-node" data-entry-slug="${esc(e.slug)}" href="${url(e)}" aria-label="打开${esc(title(e))}"><div class="compare-node-media">${im?`<img src="${esc(im.path)}" alt="${esc(im.title||title(e))}" loading="lazy">`:'<span>暂无已核验图片</span>'}</div><b>${esc(title(e))}</b><small>${esc(map.country||e.category||'')}</small><strong>${esc(map.period||m.period||'')}</strong><p>${esc(text(e))}</p><span class="compare-node-action">查看详情 →</span></a>`}
  ...
  function works(rows){if(!rows.length)return '';return `<section class="compare-works"><header><span>同期代表性器物</span><small>点击进入器物详情</small></header><div class="compare-specimen-grid">${rows.slice(0,6).map(e=>`<a class="compare-specimen" href="${url(e)}"><div class="specimen-stage">${e.media?.[0]?`<img src="${esc(e.media[0].path)}" alt="${esc(e.media[0].title||title(e))}" loading="lazy">`:'<span>暂无图片</span>'}</div><div class="specimen-meta"><b>${esc(title(e))}</b><small>${esc(meta(e).map?.country||'景德镇')}</small><span>${esc(meta(e).craft||e.category)}</span><p>${esc(text(e))}</p></div></a>`).join('')}</div></section>`}
  function render(entries){
    ...
    root.innerHTML=eras.map(([id,period,focus])=>{const related=entries.filter(e=>(meta(e).timeline||[]).some(t=>t.era===id));const jdz=related.filter(e=>(meta(e).timeline||[]).some(t=>t.era===id&&t.lane==='jdz'));const china=related.filter(e=>(meta(e).timeline||[]).some(t=>t.era===id&&t.lane==='china'));const world=related.filter(e=>(meta(e).timeline||[]).some(t=>t.era===id&&t.lane==='world'));const objs=entries.filter(e=>meta(e).kind==='object'&&(meta(e).timeline||[]).some(t=>t.era===id));return `<article class="compare-era" id="era-${id}"><div class="compare-era-heading"><div><span>${esc(period)}</span><h2>${esc(focus)}</h2></div><small>${related.length} 个历史节点</small></div><div class="compare-axis"><span>景德镇</span><i></i><span>中国其他窑业</span><i></i><span>世界</span></div>${lane('景德镇',jdz,'lane-jdz')}${lane('中国其他窑业',china,'lane-china')}${lane('世界其他地区',world,'lane-world')}${works(objs)}</article>`}).join('');
  }
