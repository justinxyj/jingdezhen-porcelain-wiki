...
(function(){
  ...
  function node(e){
    const m=meta(e),map=m.map||{},im=e.media?.[0];
    return `<a class="compare-node ${im?'has-media':'no-media'}" data-entry-slug="${esc(e.slug)}" href="${url(e)}" aria-label="打开${esc(title(e))}">
      ...
      <div class="compare-node-body"><div class="compare-node-kicker">${esc(map.country||e.category||'')}</div><b>${esc(title(e))}</b><strong>${esc(map.period||m.period||'')}</strong><p>${esc(text(e))}</p><span class="compare-node-action">查看详情 →</span></div>
    </a>`;
  }
  ...
  function works(rows){
    ...
    return `<section class="compare-works"><header><span>同期代表性器物</span><small>点击进入器物详情</small></header><div class="compare-specimen-grid">${rows.slice(0,6).map(e=>`<a class="compare-specimen" href="${url(e)}">${e.media?.[0]?`<div class="specimen-stage"><img src="${esc(e.media[0].path)}" alt="${esc(e.media[0].title||title(e))}" loading="lazy"></div>`:''}<div class="specimen-meta"><b>${esc(title(e))}</b><small>${esc(meta(e).map?.country||'景德镇')}</small><span>${esc(meta(e).craft||e.category)}</span><p>${esc(text(e))}</p></div></a>`).join('')}</div></section>`;
  }
  function render(entries){
    ...
    root.innerHTML=eras.map(([id,period,focus])=>{
      ...
      return `<article class="compare-era" id="era-${id}"><div class="compare-era-heading"><div><span>${esc(period)}</span><h2>${esc(focus)}</h2></div><small>${related.length} 个历史节点</small></div><div class="compare-axis"><span>景德镇</span><i></i><span>中国其他窑业</span><i></i><span>世界</span><i></i><span></span></div>${lane('景德镇',jdz,'lane-jdz')}${lane('中国其他窑业',china,'lane-china')}${lane('世界其他地区',world,'lane-world')}${works(objs)}</article>`;
    }).join('');
  }
