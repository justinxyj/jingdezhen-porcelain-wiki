...
(function(){
  ...
  const eras=[
    ['tang','唐 · 618–907','成熟白瓷、青瓷体系形成与跨区域交流'],
    ['song','宋 · 960–1279','多窑系并立，器物审美与技术高度分化'],
    ['yuan','元 · 1271–1368','青花与全球贸易网络发生关键变化'],
    ['ming','明 · 1368–1644','御窑制度、青花与彩瓷体系高度发展'],
    ['qing','清 · 1644–1911','粉彩、珐琅彩、颜色釉与全球消费市场扩张'],
    ['modern','近现代 · 1911–至今','工业化、教育科研、艺术陶瓷与全球设计体系']
  ];
  ...
  function node(e){
    const m=meta(e), map=m.map||{};
    return `<a class="compare-node" data-entry-slug="${esc(e.slug)}" href="${url(e)}" aria-label="打开${esc(title(e))}"><b>${esc(title(e))}</b><small>${esc(map.country||e.category||'')}</small><strong>${esc(map.period||m.period||'')}</strong><p>${esc(text(e))}</p><span class="compare-node-action">查看知识条目 →</span></a>`;
  }
  ...
  function works(rows){
    ...
    return `<section class="compare-works"><header><span>同期代表性器物</span><small>点击进入同一器物知识条目</small></header><div class="compare-specimen-grid">${rows.slice(0,6).map(e=>`<a class="compare-specimen" href="${url(e)}" data-entry-slug="${esc(e.slug)}"><div class="specimen-stage"><span class="compare-vessel compare-blue"><i></i></span></div><div class="specimen-meta"><b>${esc(title(e))}</b><small>${esc(meta(e).map?.country||'景德镇')}</small><span>${esc(meta(e).craft||e.category)}</span><p>${esc(text(e))}</p></div></a>`).join('')}</div></section>`;
  }
  function render(entries){
    ...
    root.innerHTML=eras.map(([id,period,focus])=>{
      ...
      return `<article class="compare-era" id="era-${id}"><div class="compare-era-heading"><div><span>${esc(period)}</span><h2>${esc(focus)}</h2></div><small>${related.length} 个后端知识条目</small></div><div class="compare-axis"><span>景德镇</span><i></i><span>中国其他窑业</span><i></i><span>世界</span></div>${lane('景德镇',jdz,'lane-jdz')}${lane('中国其他窑业',china,'lane-china')}${lane('世界其他地区',world,'lane-world')}${works(objs)}</article>`;
    }).join('');
  }
  async function init(){
