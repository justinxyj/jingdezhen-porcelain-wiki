...
(function(){
  ...
  async function all(){if(allPromise)return allPromise;allPromise=(async()=>{const db=client();if(!db)return[];const {data:entries,error:e}=await db.from('entries').select('id,slug,category,zh,en,ja,sources,status,version,updated_at').eq('status','published').order('updated_at',{ascending:false});if(e)throw e;const {data:media,error:me}=await db.from('media').select('id,entry_id,path,title,source,license,creator,status,created_at').eq('status','approved').order('created_at',{ascending:true});if(me)throw me;const mm=new Map();(media||[]).forEach(m=>{if(!mm.has(m.entry_id))mm.set(m.entry_id,[]);mm.get(m.entry_id).push(m)});const rows=(entries||[]).map(e=>({...e,media:mm.get(e.id)||[]}));rows.forEach(e=>cache.set(e.slug,e));return rows})().catch(err=>{console.warn('[JDM knowledge]',err);return[]});return allPromise}
