...
(function(){
  ...
  function badMedia(m){
    ...
    return /关联图|视觉索引|占位|placeholder|待补|暂无|未核验/i.test(title) || /placeholder|no[-_ ]image|noimage/i.test(path);
  }
  async function all(){
    ...
    allPromise=(async()=>{
      ...
      const {data:media,error:me}=await db.from('media').select('id,entry_id,path,title,source,license,creator,status,created_at,usage_type,source_tier,is_primary,verification_note,verified_at').eq('status','approved').order('is_primary',{ascending:false}).order('created_at',{ascending:true});if(me)throw me;
      const mm=new Map();
      (media||[]).filter(m=>!badMedia(m)).forEach(m=>{if(!mm.has(m.entry_id))mm.set(m.entry_id,[]);mm.get(m.entry_id).push(m)});
      const rows=(entries||[]).map(e=>({...e,media:mm.get(e.id)||[]}));rows.forEach(e=>cache.set(e.slug,e));return rows;
