...
(function(){
  ...
  async function list({category=null,limit=250,offset=0}={}){
    ...
    const media=window.JDM_CONTRACT.mediaList(await query((db,s)=>db.from('media_public').select('id,entry_id,path,title,source,license,creator,captured_at,location,created_at,usage_type,source_tier,is_primary,canonical_key,source_url,source_type').in('entry_id',ids).order('is_primary',{ascending:false}).order('source_tier',{ascending:true}).abortSignal(s)));
